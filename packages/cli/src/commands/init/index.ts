...
import { sampleEslintConfig, samplePrettierConfig } from './preset-config'
import { Command, Flags } from '@oclif/core'
import { prompt } from 'inquirer'
import Listr from 'listr'
import fs from 'node:fs'
import { type PackageManager, type PackageManagerName } from 'nypm'
...
export const isValidPackageManager = (
  ...
  if (!packageManager)
    throw new Error(
      'No package manager detected, are you in the right directory?'
    )
  if (['npm', 'yarn', 'pnpm'].includes(packageManager))
    return packageManager as PackageManagerName
  throw new Error(`Unsupported package manager detected: ${packageManager}`)
}
